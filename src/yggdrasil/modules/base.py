...
from yggdrasil.core.utils import lazy_property
...
class BaseModule(L.LightningModule, abc.ABC, Generic[T, STEP_GEN_T]):
    ...
    the :class:`~yggdrasil.core.dtypes.base.TensorDataClass` dataclass. To define
    ...
    def _setup_input_type(self) -> None:
        ...
        Raises:
            RuntimeError: If the `train_step_gen` method does not have a
                `training_batch` parameter, this exception is raised to indicate
                that the training data's type could not be inferred.
        """
